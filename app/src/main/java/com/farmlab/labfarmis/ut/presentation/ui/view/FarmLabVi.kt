...
class FarmLabVi(
    private val farmLabContext: Context,
    private val farmLabCallback: FarmLabCallBack,
    private val farmLabWindow: Window
) : WebView(farmLabContext) {
    private var farmLabFileChooserHandler: ((ValueCallback<Array<Uri>>?) -> Unit)? = null
    fun farmLabSetFileChooserHandler(handler: (ValueCallback<Array<Uri>>?) -> Unit) {
        this.farmLabFileChooserHandler = handler
    }
    init {
        ...
        webSettings.apply {
            ...
            userAgentString = WebSettings.getDefaultUserAgent(farmLabContext).replace("; wv)", "").replace("Version/4.0 ", "")
        ...
        super.setWebViewClient(object : WebViewClient() {
            override fun shouldOverrideUrlLoading(
                ...
                return if (request?.isRedirect == true) {
                    ...
                    try {
                        farmLabContext.startActivity(intent)
                    } catch (e: Exception) {
                        Toast.makeText(farmLabContext, "This application not found", Toast.LENGTH_SHORT).show()
                    }
                    true
                }
            }
...
//                }
        ...
        super.setWebChromeClient(object : WebChromeClient() {
            ...
            override fun onShowFileChooser(
                ...
                farmLabFileChooserHandler?.invoke(filePathCallback)
                return true
            }
            override fun onCreateWindow(
                ...
                farmLabHandleCreateWebWindowRequest(resultMsg)
                return true
            }
        })
    }
    ...
    fun farmLabFLoad(link: String) {
        super.loadUrl(link)
    }

    private fun farmLabEnableStableInputScroll() {
        evaluateJavascript(
            """
        (function() {
    ...
    private fun farmLabHandleCreateWebWindowRequest(resultMsg: Message?) {
        if (resultMsg == null) return
        if (resultMsg.obj != null && resultMsg.obj is WebView.WebViewTransport) {
            val transport = resultMsg.obj as WebView.WebViewTransport
            val windowWebView = FarmLabVi(farmLabContext, farmLabCallback, farmLabWindow)
            transport.webView = windowWebView
            resultMsg.sendToTarget()
            farmLabCallback.farmLabHandleCreateWebWindowRequest(windowWebView)
        }
    }
